...
from ops.my_resnet18_2gpu import MyResNet18
...
class TSN(nn.Module):
    def __init__(self, num_class, num_segments, modality,
                 ...
                 temporal_pool=False, non_local=False, merge=False):
    ...
    def _prepare_base_model(self, base_model):
        ...
        if 'resnet' in base_model:
            ...
            self.base_model = MyResNet18()
            self.base_model.load_state_dict(models.resnet18(pretrained=True).state_dict())
            if self.is_shift:
            ...
            if self.merge:
                print('merge frames by stage...')
                from ops.my_resnet18_2gpu import sample_merge
                sample_merge(self.base_model)
    ...
    def forward(self, input, no_reshape=False, cluster_set=None, merge=False):
        if not no_reshape:
            ...
            # print('model first view:', input.view((-1, sample_len) + input.size()[-2:]).size())

            base_out = self.base_model(input.view((-1, sample_len) + input.size()[-2:]), merge=merge)
        else:
            base_out = self.base_model(input, merge=merge)
        ...
        if self.reshape:
            if self.is_shift and self.temporal_pool:
                if cluster_set is not None:
                    cluster_set = cluster_set.view(-1, self.num_segments // 2)
                    cluster_set_shape = cluster_set.size()
                    cluster_set = cluster_set[:, :, None]
                    cluster_set = cluster_set.expand(cluster_set_shape + base_out.size()[1:])
                    print('expanded cluster:', cluster_set.shape)
                    base_out = base_out.view((-1, self.num_segments // 2) + base_out.size()[1:])
                    base_out *= cluster_set
                    print('base_out shape after:', base_out.shape)
                else:
                    base_out = base_out.view((-1, self.num_segments // 2) + base_out.size()[1:])
            ...
            if cluster_set is not None:
                output = self.consensus(base_out, cluster_set=cluster_set.sum(1, keepdim=True))
            else:
