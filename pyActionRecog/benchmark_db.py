...
def build_split_list(split_tuple, frame_info, split_idx, shuffle=False):
    ...
    print(len(split_tuple), split_idx)

    def build_set_list(set_list):
        rgb_list, flow_list = list(), list()
        for item in set_list:
            if item[0] not in frame_info[0]:
                continue
...
def parse_ucf_splits():
    class_ind = [x.strip().split() for x in open('/tudelft.net/staff-bulk/ewi/insy/VisionLab/xinliu/tmp/tsm/temporal-shift-module/UCF101/labels/classInd.txt')]
    ...
    splits = []
    for i in range(1, 2):
        train_list = [line2rec(x) for x in open('/tudelft.net/staff-bulk/ewi/insy/VisionLab/xinliu/tmp/tsm/temporal-shift-module/UCF101/labels/trainlist{:02d}.txt'.format(i))]
        test_list = [line2rec(x) for x in open('/tudelft.net/staff-bulk/ewi/insy/VisionLab/xinliu/tmp/tsm/temporal-shift-module/UCF101/labels/testlist{:02d}.txt'.format(i))]
